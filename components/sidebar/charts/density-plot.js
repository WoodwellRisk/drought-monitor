...
const DensityPlot = ({ data, variable, time, colormap }) => {
  ...
  let binEdges = Array(nBins + 1)
    .fill(0)
    .map((_, i) => Number((i * binWidth).toFixed(2)));
  ...
  let binData = (d, bin) => {
    ...
    if (checkEmptyArrays == 0) {
      // then there is no data to display
      ...
      <Chart x={[xMin, xMax]} y={[0, 100]} padding={{ left: 50, top: 0 }}>
        ...
        <Plot>
          ...
          <Area
            ...
            curve={d3.curveMonotoneX} // d3.curveBumpX is another good option
          />
          <Line data={plotData} width={1.5} color={'black'} curve={d3.curveMonotoneX} />
        </Plot>
      </Chart>
    </Box>
  );
